...
class IdealRequest implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        Assert::minLength(
            $this->name,
            3,
            "name in IdealRequest must have minlength of 3 $within"
        );
        Assert::maxLength(
            $this->name,
            300,
            "name in IdealRequest must have maxlength of 300 $within"
        );
        Assert::notNull($this->country_code, "country_code in IdealRequest must not be NULL $within");
        Assert::minLength(
            $this->country_code,
            2,
            "country_code in IdealRequest must have minlength of 2 $within"
        );
        Assert::maxLength(
            $this->country_code,
            2,
            "country_code in IdealRequest must have maxlength of 2 $within"
        );
        !isset($this->bic) || Assert::minLength(
            $this->bic,
            8,
            "bic in IdealRequest must have minlength of 8 $within"
        );
        !isset($this->bic) || Assert::maxLength(
            $this->bic,
            11,
            "bic in IdealRequest must have maxlength of 11 $within"
        );
    }
