...
class PaymentSource2 implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->token) || Assert::isInstanceOf(
            $this->token,
            Token::class,
            "token in PaymentSource2 must be instance of Token $within"
        );
        !isset($this->token) ||  $this->token->validate(PaymentSource2::class);
        !isset($this->bank) || Assert::isInstanceOf(
            $this->bank,
            Bank::class,
            "bank in PaymentSource2 must be instance of Bank $within"
        );
        !isset($this->bank) ||  $this->bank->validate(PaymentSource2::class);
        !isset($this->paypal) || Assert::isInstanceOf(
            $this->paypal,
            PaypalWallet::class,
            "paypal in PaymentSource2 must be instance of PaypalWallet $within"
        );
        !isset($this->paypal) ||  $this->paypal->validate(PaymentSource2::class);
        !isset($this->alipay) || Assert::isInstanceOf(
            $this->alipay,
            AlipayRequest::class,
            "alipay in PaymentSource2 must be instance of AlipayRequest $within"
        );
        !isset($this->alipay) ||  $this->alipay->validate(PaymentSource2::class);
        !isset($this->bancontact) || Assert::isInstanceOf(
            $this->bancontact,
            BancontactRequest::class,
            "bancontact in PaymentSource2 must be instance of BancontactRequest $within"
        );
        !isset($this->bancontact) ||  $this->bancontact->validate(PaymentSource2::class);
        ...
        !isset($this->eps) || Assert::isInstanceOf(
            $this->eps,
            EpsRequest::class,
            "eps in PaymentSource2 must be instance of EpsRequest $within"
        );
        !isset($this->eps) ||  $this->eps->validate(PaymentSource2::class);
        !isset($this->giropay) || Assert::isInstanceOf(
            $this->giropay,
            GiropayRequest::class,
            "giropay in PaymentSource2 must be instance of GiropayRequest $within"
        );
        !isset($this->giropay) ||  $this->giropay->validate(PaymentSource2::class);
        !isset($this->ideal) || Assert::isInstanceOf(
            $this->ideal,
            IdealRequest::class,
            "ideal in PaymentSource2 must be instance of IdealRequest $within"
        );
        !isset($this->ideal) ||  $this->ideal->validate(PaymentSource2::class);
        !isset($this->multibanco) || Assert::isInstanceOf(
            $this->multibanco,
            MultibancoRequest::class,
            "multibanco in PaymentSource2 must be instance of MultibancoRequest $within"
        );
        !isset($this->multibanco) ||  $this->multibanco->validate(PaymentSource2::class);
        ...
        !isset($this->payu) || Assert::isInstanceOf(
            $this->payu,
            PayuRequest::class,
            "payu in PaymentSource2 must be instance of PayuRequest $within"
        );
        !isset($this->payu) ||  $this->payu->validate(PaymentSource2::class);
        ...
        !isset($this->sofort) || Assert::isInstanceOf(
            $this->sofort,
            SofortRequest::class,
            "sofort in PaymentSource2 must be instance of SofortRequest $within"
        );
        !isset($this->sofort) ||  $this->sofort->validate(PaymentSource2::class);
        ...
        !isset($this->verkkopankki) || Assert::isInstanceOf(
            ...
            "verkkopankki in PaymentSource2 must be instance of VerkkopankkiRequest $within"
        );
        !isset($this->verkkopankki) ||  $this->verkkopankki->validate(PaymentSource2::class);
