 ...
 *
 * generated from:
 * customized_x_unsupported_2558_MerchantsCommonComponentsSpecification-v1-schema-vault_paypal_wallet_base.json
 */
class VaultPaypalWalletBase2 implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->description) || Assert::minLength(
            $this->description,
            1,
            "description in VaultPaypalWalletBase2 must have minlength of 1 $within"
        );
        !isset($this->description) || Assert::maxLength(
            $this->description,
            128,
            "description in VaultPaypalWalletBase2 must have maxlength of 128 $within"
        );
        !isset($this->product_label) || Assert::minLength(
            $this->product_label,
            1,
            "product_label in VaultPaypalWalletBase2 must have minlength of 1 $within"
        );
        !isset($this->product_label) || Assert::maxLength(
            $this->product_label,
            25,
            "product_label in VaultPaypalWalletBase2 must have maxlength of 25 $within"
        );
        Assert::notNull($this->usage_type, "usage_type in VaultPaypalWalletBase2 must not be NULL $within");
        Assert::minLength(
            $this->usage_type,
            1,
            "usage_type in VaultPaypalWalletBase2 must have minlength of 1 $within"
        );
        Assert::maxLength(
            $this->usage_type,
            255,
            "usage_type in VaultPaypalWalletBase2 must have maxlength of 255 $within"
        );
        !isset($this->customer_type) || Assert::minLength(
            $this->customer_type,
            1,
            "customer_type in VaultPaypalWalletBase2 must have minlength of 1 $within"
        );
        !isset($this->customer_type) || Assert::maxLength(
            $this->customer_type,
            255,
            "customer_type in VaultPaypalWalletBase2 must have maxlength of 255 $within"
        );
    }
