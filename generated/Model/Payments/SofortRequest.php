...
class SofortRequest implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        Assert::notNull($this->name, "name in SofortRequest must not be NULL $within");
        Assert::minLength(
            $this->name,
            3,
            "name in SofortRequest must have minlength of 3 $within"
        );
        Assert::maxLength(
            $this->name,
            300,
            "name in SofortRequest must have maxlength of 300 $within"
        );
        Assert::notNull($this->country_code, "country_code in SofortRequest must not be NULL $within");
        Assert::minLength(
            $this->country_code,
            2,
            "country_code in SofortRequest must have minlength of 2 $within"
        );
        Assert::maxLength(
            $this->country_code,
            2,
