 ...
 *
 * generated from: referred-referred_outcome.json
 */
class ReferredReferredOutcome implements JsonSerializable
{
    use BaseModel;
    ...
    public function validate($from = null)
    {
        ...
        Assert::notNull($this->faulty_party, "faulty_party in ReferredReferredOutcome must not be NULL $within");
        Assert::minLength(
            $this->faulty_party,
            1,
            "faulty_party in ReferredReferredOutcome must have minlength of 1 $within"
        );
        Assert::maxLength(
            $this->faulty_party,
            255,
            "faulty_party in ReferredReferredOutcome must have maxlength of 255 $within"
        );
        Assert::notNull($this->adjudication_reason, "adjudication_reason in ReferredReferredOutcome must not be NULL $within");
        Assert::minLength(
            $this->adjudication_reason,
            1,
            "adjudication_reason in ReferredReferredOutcome must have minlength of 1 $within"
        );
        Assert::maxLength(
            $this->adjudication_reason,
            2000,
            "adjudication_reason in ReferredReferredOutcome must have maxlength of 2000 $within"
        );
        !isset($this->resolution_date) || Assert::minLength(
            $this->resolution_date,
            20,
            "resolution_date in ReferredReferredOutcome must have minlength of 20 $within"
        );
        !isset($this->resolution_date) || Assert::maxLength(
            $this->resolution_date,
            64,
            "resolution_date in ReferredReferredOutcome must have maxlength of 64 $within"
        );
    }
