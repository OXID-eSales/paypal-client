 ...
 */
class ReferredTrackingInfoItem implements JsonSerializable
{
    use BaseModel;
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->carrier_name) || Assert::minLength(
            $this->carrier_name,
            1,
            "carrier_name in ReferredTrackingInfoItem must have minlength of 1 $within"
        );
        !isset($this->carrier_name) || Assert::maxLength(
            $this->carrier_name,
            2000,
            "carrier_name in ReferredTrackingInfoItem must have maxlength of 2000 $within"
        );
        !isset($this->tracking_url) || Assert::minLength(
            $this->tracking_url,
            1,
            "tracking_url in ReferredTrackingInfoItem must have minlength of 1 $within"
        );
        !isset($this->tracking_url) || Assert::maxLength(
            $this->tracking_url,
            2000,
            "tracking_url in ReferredTrackingInfoItem must have maxlength of 2000 $within"
        );
        !isset($this->tracking_number) || Assert::minLength(
            $this->tracking_number,
            1,
            "tracking_number in ReferredTrackingInfoItem must have minlength of 1 $within"
        );
        !isset($this->tracking_number) || Assert::maxLength(
            $this->tracking_number,
            255,
            "tracking_number in ReferredTrackingInfoItem must have maxlength of 255 $within"
        );
        !isset($this->tracking_status) || Assert::minLength(
            $this->tracking_status,
            1,
            "tracking_status in ReferredTrackingInfoItem must have minlength of 1 $within"
        );
        !isset($this->tracking_status) || Assert::maxLength(
            $this->tracking_status,
            255,
            "tracking_status in ReferredTrackingInfoItem must have maxlength of 255 $within"
        );
        !isset($this->note) || Assert::minLength(
            $this->note,
            1,
            "note in ReferredTrackingInfoItem must have minlength of 1 $within"
        );
        !isset($this->note) || Assert::maxLength(
            $this->note,
            2000,
            "note in ReferredTrackingInfoItem must have maxlength of 2000 $within"
        );
        !isset($this->posted_time) || Assert::minLength(
            $this->posted_time,
            20,
            "posted_time in ReferredTrackingInfoItem must have minlength of 20 $within"
        );
        !isset($this->posted_time) || Assert::maxLength(
            $this->posted_time,
            64,
            "posted_time in ReferredTrackingInfoItem must have maxlength of 64 $within"
        );
    }
