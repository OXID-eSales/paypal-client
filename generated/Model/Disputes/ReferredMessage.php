 ...
 */
class ReferredMessage implements JsonSerializable
{
    use BaseModel;
    ...
    public function validate($from = null)
    {
        ...
        Assert::notNull($this->posted_by, "posted_by in ReferredMessage must not be NULL $within");
        Assert::minLength(
            $this->posted_by,
            1,
            "posted_by in ReferredMessage must have minlength of 1 $within"
        );
        Assert::maxLength(
            $this->posted_by,
            255,
            "posted_by in ReferredMessage must have maxlength of 255 $within"
        );
        !isset($this->posted_time) || Assert::minLength(
            $this->posted_time,
            20,
            "posted_time in ReferredMessage must have minlength of 20 $within"
        );
        !isset($this->posted_time) || Assert::maxLength(
            $this->posted_time,
            64,
            "posted_time in ReferredMessage must have maxlength of 64 $within"
        );
        Assert::notNull($this->content, "content in ReferredMessage must not be NULL $within");
        Assert::maxLength(
            $this->content,
            2000,
            "content in ReferredMessage must have maxlength of 2000 $within"
        );
    }
