...
namespace OxidSolutionCatalysts\PayPal\Api\Model\Catalog;
...
class ProductRequestPOST implements JsonSerializable
{
    ...
    public function validate($from = null)
    {
        ...
        !isset($this->id) || Assert::minLength(
            $this->id,
            6,
            "id in ProductRequestPOST must have minlength of 6 $within"
        );
        !isset($this->id) || Assert::maxLength(
            $this->id,
            50,
            "id in ProductRequestPOST must have maxlength of 50 $within"
        );
        Assert::notNull($this->name, "name in ProductRequestPOST must not be NULL $within");
        Assert::minLength(
            $this->name,
            1,
            "name in ProductRequestPOST must have minlength of 1 $within"
        );
        Assert::maxLength(
            $this->name,
            127,
            "name in ProductRequestPOST must have maxlength of 127 $within"
        );
        !isset($this->description) || Assert::minLength(
            $this->description,
            1,
            "description in ProductRequestPOST must have minlength of 1 $within"
        );
        !isset($this->description) || Assert::maxLength(
            $this->description,
            256,
            "description in ProductRequestPOST must have maxlength of 256 $within"
        );
        Assert::notNull($this->type, "type in ProductRequestPOST must not be NULL $within");
        Assert::minLength(
            $this->type,
            1,
            "type in ProductRequestPOST must have minlength of 1 $within"
        );
        Assert::maxLength(
            $this->type,
            24,
            "type in ProductRequestPOST must have maxlength of 24 $within"
        );
        !isset($this->category) || Assert::minLength(
            $this->category,
            4,
            "category in ProductRequestPOST must have minlength of 4 $within"
        );
        !isset($this->category) || Assert::maxLength(
            $this->category,
            256,
            "category in ProductRequestPOST must have maxlength of 256 $within"
        );
        !isset($this->image_url) || Assert::minLength(
            $this->image_url,
            1,
            "image_url in ProductRequestPOST must have minlength of 1 $within"
        );
        !isset($this->image_url) || Assert::maxLength(
            $this->image_url,
            2000,
            "image_url in ProductRequestPOST must have maxlength of 2000 $within"
        );
        !isset($this->home_url) || Assert::minLength(
            $this->home_url,
            1,
            "home_url in ProductRequestPOST must have minlength of 1 $within"
        );
        !isset($this->home_url) || Assert::maxLength(
            $this->home_url,
            2000,
            "home_url in ProductRequestPOST must have maxlength of 2000 $within"
        );
    }
